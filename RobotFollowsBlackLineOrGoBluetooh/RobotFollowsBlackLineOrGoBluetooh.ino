...
void re_Trai(int speed, int PID_value){
  motor_Phai_Tien(speed + PID_value); // motor 1 tiến
	motor_Trai_Lui(speed - PID_value);
}
...
int read_Sensor_Values(){
  ...
  */
  ...
  if((leftValue == 0)&&(midValue == 0)&&(rightValue == 0)){// Truong hop lech khoi Vach Den
    ...
    error = 2;  
  }else if((leftValue == 0)&&(midValue == 1)&&(rightValue == 0)){
    error = 0;
  }else if((leftValue == 0)&&(midValue == 1)&&(rightValue == 1)){
    error = 1;
  }else if((leftValue == 1)&&(midValue == 0)&&(rightValue == 0)){
    error = -2;   
  }else if((leftValue == 1)&&(midValue == 0)&&(rightValue == 1)){
    di_Lui(speed);
...
void do_Duong(){
  ...
  moto_Control(200, PID_value);
}

void ESP_run(char signal){
  switch (signal) {
    case 'w':
      di_Thang(150);
      delay(500);
